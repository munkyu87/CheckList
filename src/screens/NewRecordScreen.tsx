...
export function NewRecordScreen() {
      ...
      navigation.setOptions({
        headerLeft: () => (
          <Pressable onPress={() => setStep('form')} style={{ paddingVertical: 8, paddingRight: 16 }}>
            <Text style={{ fontSize: 16, color: theme.primary }}>이전</Text>
          </Pressable>
        ),
      });
    } else {
  ...
  const groupOptions = useMemo(
    () => [{ id: null as string | null, name: '직접 작성' }, ...groups.map(g => ({ id: g.id, name: g.name }))],
    [groups]
  );
    ...
    return (
      ...
      >
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.sectionTitle}>체크리스트 유형</Text>
          <View style={{ marginBottom: 8 }}>
            ...
            >
              <Text style={styles.comboTriggerText}>
                {groupId === null ? '직접 작성' : (selectedGroup?.name ?? '선택')}
              </Text>
              <Text style={styles.comboArrow}>{'>'}</Text>
            </Pressable>
          </View>
          ...
          <Pressable
            ...
            disabled={!canGoNext}
          >
            <Text style={styles.btnPrimaryText}>기록 시작</Text>
          </Pressable>
        </ScrollView>
        <BottomSheet
          ref={groupSheetRef}
    ...
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.row}>
          <Text style={styles.rowLabel}>{subjectLabel}</Text>
          <Text style={styles.rowValue}>{subjectName}</Text>
          <Text style={[styles.rowLabel, { marginTop: 4 }]}>{formatDate(date)} · {selectedGroup?.name ?? '직접 작성'}</Text>
        </View>
